"""
Korean PII Detection Validation Script.
    ...
    for test_case, expected_label in dataset:
        ...
        for token in expected_label.split():
            if '<' in token and '>' in token:
                # Extract entity type from token like "<KR_NAME>님의"
                start = token.find('<')
                end = token.find('>') + 1
...
def print_report(evaluation: Dict):
    ...
    print("=" * 70)
    print("PII MASKING EVALUATION REPORT - KOREAN")
    print("=" * 70)
...
def main():
    ...
    evaluation = evaluate_masking(shield, dataset, language="ko")
